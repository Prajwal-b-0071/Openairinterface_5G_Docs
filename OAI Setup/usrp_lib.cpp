...
//below according to chatgpt is not used anywhere
#include <sys/resource.h> //includes the POSIX system resource management definitions — mainly used to control and query resource limits such as CPU scheduling priority, stack size, memory limits, etc.OAI attempts to reduce latency and avoid scheduling delays by setting higher priority for critical TX/RX threads using functions

#include "openair1/PHY/sse_intrin.h" //this function - helps ode shifts each 16-bit IQ sample left by 4 bits using SIMD (vectorized) instructions. Used in -trx_usrp_write()
...
//is used to check whether the USRP's reference clock input has successfully locked-This could include sensors like "ref_locked", "gps_locked", "lo_locked", etc.this function checkes if all the sensors are locked to the clock
...
static int sync_to_gps(openair0_device *device) {
  ...
  usrp_state_t *s = (usrp_state_t *)device->priv;//Retrieves the USRP state (s) associated with this OAI radio device. s->usrp gives access to the UHD API.

  try {
    size_t num_mboards = s->usrp->get_num_mboards(); // get number of mother boards
    size_t num_gps_locked = 0; // track how many boards are GPS locked
    ...
    for (size_t mboard = 0; mboard < num_mboards; mboard++) {
      ...
      //Set to GPS time
      uhd::time_spec_t gps_time = uhd::time_spec_t(time_t(s->usrp->get_mboard_sensor("gps_time", mboard).to_int())); //Fetch current GPS time from the GPSDO
      s->usrp->set_time_next_pps(gps_time+1.0, mboard); //Schedule the USRP’s time to be set to that GPS time at the next PPS (Pulse Per Second) edge
...
static void trx_usrp_start_interdigital_gpio(openair0_device *device, usrp_state_t *s)
{
  ...
  s->usrp->set_gpio_attr(s->gpio_bank, "DDR", 0xfff, 0xfff); //Sets GPIO pins as outputs
...
static void trx_usrp_start_generic_gpio(openair0_device *device, usrp_state_t *s) //Prepares the GPIOs to switch external RF front-end components based on TX/RX state
{
...
*/
static int trx_usrp_start(openair0_device *device) { //Starts and configures the USRP radio hardware before actual transmission or reception.
  ...
  switch (device->openair0_cfg->gpio_controller) {
    case RU_GPIO_CONTROL_NONE:  //no gpio logic
      break;
    case RU_GPIO_CONTROL_GENERIC: //Standard TDD Pins
      trx_usrp_start_generic_gpio(device, s);
      break;
    case RU_GPIO_CONTROL_INTERDIGITAL: //For advanced beamforming control
      trx_usrp_start_interdigital_gpio(device, s);
      break;
    default:
      AssertFatal(false, "illegal GPIO controller %d\n", device->openair0_cfg->gpio_controller);
  }
//the below is reseting the timing and counters ,to track the PPS sync,count TX/RX packets/record timestamp offset for sample scheduling 
  ...
  s->rx_timestamp = 0;
    ...
    //wait for next pps-pulse per second
  ...
  LOG_I(HW,"current pps at %f, starting streaming at %f\n",current_pps.get_real_secs(),current_pps.get_real_secs()+1.0); //Just logs that PPS was captured and RX will start 1 second later
//Schedule RX Stream Start-Create continuous RX stream command
  uhd::stream_cmd_t cmd(uhd::stream_cmd_t::STREAM_MODE_START_CONTINUOUS);
  cmd.time_spec = uhd::time_spec_t(current_pps+1.0);
  cmd.stream_now = false; // start at constant delay
  s->rx_stream->issue_stream_cmd(cmd); //Starts the stream from the USRP
//This ensures that RX streaming starts on a synchronized boundary, critical for PHY-layer timing.
  return 0;
}

static void trx_usrp_send_end_of_burst(usrp_state_t *s) //this tells-I'm done transmitting data. This was the end of the burst
{
...
static void trx_usrp_finish_rx(usrp_state_t *s) //This function, trx_usrp_finish_rx, is used to cleanly stop reception (RX streaming) from the USRP device. It's typically called when shutting down the radio or reinitializing the RX chain.
{
  ...
  size_t samples;
  uint8_t buf[1024]; //Temporary buffer to collect leftover data still in the USRP RX buffers.
  std::vector<void *> buff_ptrs; //A vector of void pointers — required because recv() expects an array of pointers (one for each channel).
  for (size_t i = 0; i < s->usrp->get_rx_num_channels(); i++) buff_ptrs.push_back(buf); //Iterates over each RX channel and assigns the same buffer pointer.
  do { //Receive Leftovers Until Empty
...
static void trx_usrp_end(openair0_device *device) {
  ...
  iqrecorder_end(device); //If IQ recording was active (for debugging or tracing), this stops the recording and flushes any saved files.
...
static int trx_usrp_write(openair0_device *device,
			  ...
			  void **buff,
			  int nsamps, //Number of complex baseband samples per channel to write.
			  int cc, //Component Carrier index or number of antennas/channels.
			  int flags) {//A bitmask or set of flags (defined by OAI) that indicate:1: Start of burst,2: End of burst4: Has timestamp
  int ret=0;
  usrp_state_t *s = (usrp_state_t *)device->priv;
  timestamp -= device->openair0_cfg->command_line_sample_advance + device->openair0_cfg->tx_sample_advance; //Adjust the transmission time to pre-send the samples in advance.OAI pre-calculates samples ahead of time to account for:-Latency in sample transport-DSP pipeline delay-Buffering inside the USRP.The subtraction makes sure that the sample hits the air at exactly the desired slot/subframe timing.
  int nsamps2;  // aligned to upper 32 or 16 byte boundary

  radio_tx_burst_flag_t flags_burst = (radio_tx_burst_flag_t) (flags & 0xf); //Extracts the lower 4 bits of the flags parameter.->Start-of-burst->End-of-burst->Has timestamp->Possibly more, depending on radio_tx_burst_flag_t enum.
  radio_tx_gpio_flag_t flags_gpio = (radio_tx_gpio_flag_t) ((flags >> 4) & 0x1fff);//Shifts flags right by 4 bits, then masks with 0x1fff (13 bits).This isolates the GPIO control flags, used to:1)Change TX/RX mode 2)Control GPIO pins on the USRP

  int end;
  openair0_thread_t *write_thread = &device->write_thread; //Retrieves the TX write thread from the device.
  openair0_write_package_t *write_package = write_thread->write_package;//Accesses the thread’s ring-buffer-like structure for managing outgoing TX data.

  AssertFatal( MAX_WRITE_THREAD_BUFFER_SIZE >= cc,"Do not support more than %d cc number\n", MAX_WRITE_THREAD_BUFFER_SIZE); //Ensure we don’t exceed the write thread’s maximum number of supported channels (component carriers).
  ...
  bool first_packet_state=false,last_packet_state=false;
    ...
    if (usrp_tx_thread == 0) {
      nsamps2 = (nsamps+7)>>3; //Rounds nsamps to nearest 8 multiple (for 256-bit AVX alignment).
      simde__m256i buff_tx[cc < 2 ? 2 : cc][nsamps2]; //Local aligned AVX buffer for TX samples per component carrier (CC).AVX registers (simde__m256i) hold 16 samples of 16-bit I/Q data per block.

      // bring RX data into 12 LSBs for softmodem RX
      for (int i = 0; i < cc; i++) {
        for (int j = 0; j < nsamps2; j++) {
          if ((((uintptr_t)buff[i]) & 0x1F) == 0) {
            buff_tx[i][j] = simde_mm256_slli_epi16(((simde__m256i *)buff[i])[j], 4);//If buff[i] is 32-byte aligned: direct AVX load + shift left by 4 bits.
          } else {//use unaligned load, then shift.
...
//below sends samples to UHD.If single carrier: simple pointer send.Multi-CC: use std::vector for UHD multi-channel send
    if (cc>1) {
...
//Store Packet Info in Ring Buffer
...
/*! \brief Called to send samples to the USRP RF target
      ...
      @param flags flags must be set to true if timestamp parameter needs to be applied
*/
void *trx_usrp_write_thread(void * arg){ //this is the consumer thread that reads I/Q data from the ring buffer (populated by trx_usrp_write) and sends it to the USRP
  int ret=0;
  openair0_device *device=(openair0_device *)arg;
  openair0_thread_t *write_thread = &device->write_thread;
  openair0_write_package_t *write_package = write_thread->write_package; //circular buffer of tx package

  usrp_state_t *s; //usrp driver state
  int nsamps2;  // aligned to upper 32 or 16 byte boundary
  int start;
  openair0_timestamp timestamp;
  void               **buff;
  ...
  while(1){
    pthread_mutex_lock(&write_thread->mutex_write); //Acquires exclusive access to the ring buffer (blocking write thread).
    ...
    /*if(write_thread->count_write != 0){
      LOG_W(HW,"count write = %d, start = %d, end = %d\n", write_thread->count_write, write_thread->start, write_thread->end);
        ...
        nsamps2 = (nsamps+7)>>3;
        simde__m256i buff_tx[cc < 2 ? 2 : cc][nsamps2]; //Declares a 2D buffer to hold the preprocessed I/Q data for each component carrier (cc).

        // bring RX data into 12 LSBs for softmodem RX.Goal: Shift all I/Q 16-bit samples left by 4 bits — i.e., sample <<= 4 (multiply by 16).Moves values into the 12 LSBs, as required by OAI PHY layer and USRP.
        for (int i = 0; i < cc; i++) {
          for (int j = 0; j < nsamps2; j++) {
            if ((((uintptr_t) buff[i])&0x1F)==0) {
    ...
    //Transmit the Samples-If multi-carrier (cc > 1): send vector of buffers (UHD supports this).- Else: send single I/Q buffer.
    if (cc>1) {
      std::vector<void *> buff_ptrs;
...
//Trace and Validate Transmission
    T(T_USRP_TX_ANT0, T_INT(timestamp), T_BUFFER(buff_tx[0], nsamps*4));

    if (ret != nsamps) LOG_E(HW,"[xmit] tx samples %d != %d\n",ret,nsamps); //Ensures number of sent samples matches requested. Mismatch may mean TX underrun, buffer overflow, or hardware issues.
...
//The function trx_usrp_write_init(openair0_device *device) initializes the components necessary to transmit data from OAI (OpenAirInterface) to the USRP hardware using a dedicated transmission (TX) thread.
int trx_usrp_write_init(openair0_device *device){
  ...
  //Spawns the actual TX thread (trx_usrp_write_thread) using the helper threadCreate.
...
//The function trx_usrp_write_reset(openair0_thread_t *wt) is a synchronization and cleanup routine specifically designed to terminate the USRP write thread gracefully in the usrp_lib module of OpenAirInterface (OAI).
static void trx_usrp_write_reset(openair0_thread_t *wt) {
  pthread_mutex_lock(&wt->mutex_write); //Locks the mutex guarding the TX thread’s ring buffer and shared state.Ensures atomic update of the thread control variables below.Prevents race conditions if the TX thread is actively using the buffer.
  wt->count_write = 1; //Artificially sets the write count to 1, even if it was 0.Purpose: ensures that the trx_usrp_write_thread will wake up and process at least one iteration (even if it's just to see the exit flag).
  wt->write_thread_exit = true; //Sets the exit flag, which tells the write thread that it should exit cleanly at the next opportunity.
  pthread_cond_signal(&wt->cond_write); //Wakes up the write thread, in case it was sleeping inside
  pthread_mutex_unlock(&wt->mutex_write); //Unlocks the mutex, allowing the write thread to proceed and exit.
  void *retval = NULL;
...
static int trx_usrp_read(openair0_device *device, openair0_timestamp *ptimestamp, void **buff, int nsamps, int cc) {
  usrp_state_t *s = (usrp_state_t *)device->priv; //gets USRP device
  ...
  for (int i=0; i<cc; i++) {
    for (int j = 0; j < nsamps2; j++) {
      ...
      if ((((uintptr_t) buff[i])&0x1F)==0) { //Shifts and aligns the received raw IQ samples into the proper 12-bit format.Uses AVX2 SIMD instructions for efficiency.
  ...
  //Error Handling and Timestamping
...
void *freq_thread(void *arg) {
  ...
  uhd::tune_request_t tx_tune_req(device->openair0_cfg[0].tx_freq[0],
                                  device->openair0_cfg[0].tune_offset);//uhd::tune_request_t is a UHD (USRP Hardware Driver) struct that tells the USRP what frequency to tune to.tx_freq and rx_freq are the center frequencies for transmission and reception respectively.
  uhd::tune_request_t rx_tune_req(device->openair0_cfg[0].rx_freq[0],
                                  device->openair0_cfg[0].tune_offset);//tune_offset adjusts the digital tuning frequency (e.g., to avoid DC offset at 0 Hz baseband).
...
static void usrp_sync_pps(usrp_state_t *s) //This function synchronizes the internal USRP hardware clock with the host system clock at the next PPS (Pulse Per Second) event. PPS is typically used in conjunction with GPSDO for high-precision timing.
{
  // First, wait for PPS.
  uhd::time_spec_t time_last_pps = s->usrp->get_time_last_pps();
  ...
  s->usrp->set_time_next_pps(uhd::time_spec_t(tai_sec)); //Schedules the USRP’s internal hardware clock to be updated to the host’s TAI time at the next PPS edge.
...
extern "C" {
  int device_init(openair0_device *device, openair0_config_t *openair0_cfg) {
    ...
    //If USRP is being initialized in record mode (to capture and store samples), a message is printed.
    if (device->openair0_cfg->recplay_mode == RECPLAY_RECORDMODE) {
      std::cerr << "USRP device initialized in subframes record mode" << std::endl;
    }
    //Logs the parsed version for tracking and compatibility checks (important for OAI to behave correctly with different UHD versions).
    ...
    LOG_I(HW,"Found USRP %s\n", device_adds[0].get(type_str).c_str());//Gets the info of the connected USRP device 
    double usrp_master_clock;
    //for B200 series clock initialization
    ...
    s->usrp = uhd::usrp::multi_usrp::make(args); //Calls the UHD API to instantiate the USRP object with the built argument string
...
//the USRP device is of type USRP_B200_DEV, and:Loads the correct calibration table.,Sets the master clock rate.,Configures sample advance and bandwidth settings based on the sampling rate.
  ...
  //Specifies which TX chain on the USRP to use (e.g., "A:0").
  ...
  //This loop initializes parameters for each RX channel on the USRP.
  if(openair0_cfg[0].rx_subdev!=NULL){ //We only configure a channel if it is marked as active in config.
  ...
  for(int i=0; i<((int) s->usrp->get_rx_num_channels()); i++) {
    ...
    if (i < cfg->rx_num_channels) {
      s->usrp->set_rx_rate(cfg->sample_rate, i + choffset); //This sets the sampling rate (e.g., 30.72 MHz) for each RX channel.
      uhd::tune_request_t rx_tune_req(cfg->rx_freq[i], cfg->tune_offset);
      s->usrp->set_rx_freq(rx_tune_req, i+choffset); //Tuning: Set RX center frequency per channel using rx_freq[i] and optional tune_offset for LO (Local Oscillator) control.
      set_rx_gain_offset(cfg, i, bw_gain_adjust); //Gain Offset: This corrects hardware-specific gain offsets.
      ::uhd::gain_range_t gain_range = s->usrp->get_rx_gain_range(i+choffset); //Gain Range: Retrieves the allowed min/max gain values.
      // limit to maximum gain
      double gain = cfg->rx_gain[i] - cfg->rx_gain_offset[i]; //Applies the corrected gain while clipping to maximum if necessary.
  ...
  //Logs the number of available TX channels on the USRP hardware.
  ...
  //Checks whether the clock_source parameter was passed in the USRP device string (e.g., type=b200,clock_source=external).
  ...
  uhd::stream_args_t stream_args_rx("sc16", "sc16"); //"sc16" means short complex 16-bit samples for both CPU and wire formats (I/Q data as 16-bit integers).This is a common format used in SDR systems due to its bandwidth vs. precision trade-off.
  //Configures which RX channels to use based on the software configuration.
  for (int i = 0; i<openair0_cfg[0].rx_num_channels; i++) {
    LOG_I(HW,"setting rx channel %d\n",i+choffset);
    stream_args_rx.channels.push_back(i+choffset);
  }
  //This establishes the RX stream between the host and USRP using the specified parameters.
  s->rx_stream = s->usrp->get_rx_stream(stream_args_rx);
  //pulls the configured sample rate 
  int samples=openair0_cfg[0].sample_rate;
  int max=s->rx_stream->get_max_num_samps(); //Divides by 10,000 to get number of samples for 100µs worth of data.For example, 30.72e6 / 10,000 = 3,072 samples per 100µs.
  samples/=10000;
  LOG_I(HW,"RF board max packet size %u, size for 100µs jitter %d \n", max, samples);
  //If the number of samples needed for 100µs is less than the hardware’s max, set it explicitly.
  if ( samples < max ) {
    stream_args_rx.args["spp"] = str(boost::format("%d") % samples ); //This reduces latency and jitter by controlling the streaming buffer size.
  }

  LOG_I(HW,"rx_max_num_samps %zu\n",
        s->rx_stream->get_max_num_samps());

  uhd::stream_args_t stream_args_tx("sc16", "sc16");//use 16-bit complex format for transmission.
